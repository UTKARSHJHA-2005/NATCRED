// This is the component used in Home for pricing of TradeAssure.
import { FaCheckCircle } from "react-icons/fa";// Icon
// Plans for TradeAssure
const plans = [
  {
    name: "Basic",
    price: "$99",
        ...
        <div className="grid max-w-sm grid-cols-1 gap-6 mx-auto mt-8 text-center md:text-left md:mt-16 md:max-w-6xl md:grid-cols-3">
          {plans.map((plan, index) => (
            <div className="relative" key={index}>
              {plan.gradient && (
                <div className="absolute inset-0 w-full h-full opacity-30 blur-lg filter" style={{ background: "linear-gradient(90deg, #44ff9a -0.55%, #44b0ff 22.86%, #8b44ff 48.36%, #ff6644 73.33%, #ebff70 99.34%)" }}></div>)}
              <div data-aos="flip-right" className="relative overflow-hidden bg-white border-[4px] hover:scale-110 transition-ease-all duration-300 border-green-300 rounded-2xl ">
                <div className="p-6 lg:px-10 lg:py-8">
                  ...
                  <a href="#" className={`inline-flex items-center justify-center px-8 py-3.5 w-full mt-8 text-base font-bold transition-all duration-200 rounded-xl ${plan.buttonClass}`}
                   role="button">
                    Get Started
                  </a>
